...
func TestSkillsSearch(t *testing.T) {
	...
	mg.onRequest = func(conn *websocket.Conn, req protocol.Request) {
		if req.Method == "skills.search" {
			r := protocol.SkillsSearchResult{Results: []protocol.SkillsSearchItem{
				{Score: 0.95, Slug: "test-skill", DisplayName: "Test Skill"},
			}}
			respData, _ := protocol.MarshalResponse(req.ID, r)
			conn.WriteMessage(websocket.TextMessage, respData)
		}
	...
	r, err := client.SkillsSearch(ctx, protocol.SkillsSearchParams{Query: "test"})
	if err != nil {
		t.Fatal(err)
	}
	if len(r.Results) != 1 {
	...
	tm := &testMethod{t: t, method: "skills.search", success: func(c *Client, ctx context.Context) error {
		_, err := c.SkillsSearch(ctx, protocol.SkillsSearchParams{})
		return err
	}}
	tm.run()
}
...
func TestSkillsDetail(t *testing.T) {
	...
	mg.onRequest = func(conn *websocket.Conn, req protocol.Request) {
		if req.Method == "skills.detail" {
			...
			r := protocol.SkillsDetailResult{Skill: skill}
			respData, _ := protocol.MarshalResponse(req.ID, r)
			conn.WriteMessage(websocket.TextMessage, respData)
		}
	}
	...
	r, err := client.SkillsDetail(ctx, protocol.SkillsDetailParams{Slug: "test-skill"})
	if err != nil {
		t.Fatal(err)
	}
	if r.Skill == nil || r.Skill.Slug != "test-skill" {
		t.Errorf("unexpected skill: %+v", r.Skill)
	}

	tm := &testMethod{t: t, method: "skills.detail", success: func(c *Client, ctx context.Context) error {
		_, err := c.SkillsDetail(ctx, protocol.SkillsDetailParams{Slug: "test-skill"})
		return err
	}}
	tm.run()
}
